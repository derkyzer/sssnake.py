...
save_cheat_scores = True	## Scores will save while cheats are enabled.
...
class TextObj:
	def __init__(self,window_size,string,color=fg_color,y_offset=0,string_additions=[],is_bold=False,relative_font_size=1):
		...
		self.color = color
		self.font = pygame.font.SysFont(font_name,round((grid_size[0]*box_size*relative_font_size)/10),bold=is_bold)
...
class SnakeObj:
	...
	def move(self,td=speed,direction=False): ## Probably the most complex function.
		...
		if direction:
			...
			elif direction in deltas and self.max_length==1 or god_mode or self.direction != {"up":"down","down":"up","left":"right","right":"left"}[direction]:
		...
		if self.can_move and not self.dying:
			...
			if self.direction in deltas:
				...
				new_head = [[[[grid_size[xy]-1,0][self.head[xy]+delta[xy]<=0],[self.head[xy]+delta[xy],[0,grid_size[xy]-1][self.head[xy]+delta[xy]==-1]][self.head[xy]+delta[xy] in [-1,grid_size[xy]]]][loop_screen],self.head[xy]+delta[xy]][0<self.head[xy]+delta[xy]<grid_size[xy]-1] for xy in range(2)]

				if new_head in self.body and not god_mode: return(self.die())
	...
	def die(self,force=False):
		...
		if not force and not self.dying and immortal and True in [[[[[grid_size[xy]-1,0][self.head[xy]+deltas[delta][xy]<=0],[self.head[xy]+deltas[delta][xy],[0,grid_size[xy]-1][self.head[xy]+deltas[delta][xy]==-1]][self.head[xy]+deltas[delta][xy] in [-1,grid_size[xy]]]][loop_screen],self.head[xy]+deltas[delta][xy]][0<self.head[xy]+deltas[delta][xy]<grid_size[xy]-1] for xy in range(2)] not in self.body for delta in deltas]:
			return()
		else:
	...
	def update(self,box_list=[]):
		...
		if self.snake.direction and not (self.snake.paused or self.paused):	## What happens during the actual game.
			...
			if self.snake.paused or self.paused:
				...
				if self.screen != "start":	## Only displaying it once so the screen isn't being constantly redrawn.
					self.screen = "start"
